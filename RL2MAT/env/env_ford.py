...
class FordEnv(gym.Env):
    ...
    def __init__(self, modelName='tracking', discrete=True, render=True):
    ...
    def step(self, action):
        ...
        if action is not None:
            obs_new = run_step(self.eng, self.modelName, action)

        if self.render:
            self.render()


        return encoded_state, self.last_reward, self.terminal_state, {
            "closed": self.closed}
